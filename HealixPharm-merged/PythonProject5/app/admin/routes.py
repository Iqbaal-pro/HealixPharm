...
@router.get("/prescriptions/queue", response_model=List[schemas.PrescriptionQueueItem])
def list_prescription_queue(db: Session = Depends(get_db)):
    """
    List all orders needing prescription verification (status PENDING_VERIFICATION).
    """
    orders = db.query(models.Order).filter(
        models.Order.status == "PENDING_VERIFICATION"
    ...
    for o in orders:
        ...
        if o.prescription and o.prescription.s3_key:
            try:
                presc_url = s3_service.generate_presigned_url(o.prescription.s3_key)
            except Exception as e:
                logger.error(f"Failed to generate presigned URL for order {o.token}: {e}")
                presc_url = o.prescription.s3_url # Fallback to stored URL if generation fails
