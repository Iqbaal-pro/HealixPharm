                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {pending.map(p => (
                        <div key={p.id} onClick={() => { setSelected(p); setStep("save"); }}
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ color: "#f1f5f9", fontSize: 13.5, fontWeight: 600 }}>Prescription <span style={{ color: "#38bdf8" }}>#{p.id.slice(-6)}</span></div>
                            {p.phone_number && <div style={{ color: "#475569", fontSize: 12 }}>{p.phone_number}</div>}
                            <div style={{ color: "#334155", fontSize: 12 }}>{new Date(p.received_at).toLocaleString()}</div>
                          </div>
                ...
                <div style={glassCard}>
                  ...
                  {selected && (
                    <div style={{ background: "rgba(6,13,26,0.8)", borderWidth: 1, borderStyle: "solid", borderColor: "rgba(148,163,184,0.07)", borderRadius: 12, overflow: "hidden", marginBottom: 18, minHeight: 180, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {selected.image_url
                        ? <img src={selected.image_url} alt="prescription" style={{ maxWidth: "100%", maxHeight: 320, objectFit: "contain" }} />
                        : <div style={{ textAlign: "center", padding: 32 }}>
                            <div style={{ fontSize: 32, opacity: 0.2, marginBottom: 8 }}>🖼</div>
                            <p style={{ color: "#334155", fontSize: 12, margin: 0, fontFamily: "monospace" }}>{selected.id}</p>
                          </div>
                      }
                    </div>
                  )}
