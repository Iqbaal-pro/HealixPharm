...
export default function SignupPage() {
  ...
  // ── Form state mirrors SignupRequest exactly ──
  const [form, setForm] = useState({
    ...
    password:  "",
    confirm_pw: "",
    // Step 2 — pharmacy required
    pharmacy_name: "",
    contact_number: "",
    whatsapp_number: "",
    address: "",
    // Step 3 — pharmacy optional
    opening_hours: "",
  ...
  const fb = (k: string): React.CSSProperties =>
    focused === k
      ? { border: "1px solid rgba(56,189,248,0.5)", boxShadow: "0 0 0 3px rgba(56,189,248,0.07)" }
      : {};
              ...
              <div className="step-in">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <Field label="Username">
                    <input type="text" placeholder="e.g. kasun_pharm" value={form.username} onChange={set("username")} onFocus={() => setFocused("un")} onBlur={() => setFocused(null)} style={{ ...inputSt, ...fb("un") }} autoComplete="username" />
                    {form.username && !usernameOk(form.username) && <Hint err>At least 3 characters required.</Hint>}
                  </Field>

                  <Field label="Email">
                    <input type="email" placeholder="you@pharmacy.lk" value={form.email} onChange={set("email")} onFocus={() => setFocused("em")} onBlur={() => setFocused(null)} style={{ ...inputSt, ...fb("em") }} autoComplete="email" />
                    {form.email && !emailOk(form.email) && <Hint err>Enter a valid email address.</Hint>}
                  </Field>

                  <Field label="Password">
                    <div style={{ position: "relative" }}>
                      <input type={showPw ? "text" : "password"} placeholder="Min. 8 characters" value={form.password} onChange={set("password")} onFocus={() => setFocused("pw")} onBlur={() => setFocused(null)} style={{ ...inputSt, paddingRight: 44, ...fb("pw") }} autoComplete="new-password" />
                  ...
                  <Field label="Confirm Password">
                    <div style={{ position: "relative" }}>
                      <input type={showCPw ? "text" : "password"} placeholder="Repeat password" value={form.confirm_pw} onChange={set("confirm_pw")} onFocus={() => setFocused("cpw")} onBlur={() => setFocused(null)} style={{ ...inputSt, paddingRight: 44, ...(form.confirm_pw && form.confirm_pw !== form.password ? { border: "1px solid rgba(248,113,113,0.5)" } : fb("cpw")) }} autoComplete="new-password" />
                ...
                <button onClick={() => step1Ok && setStep(2)} disabled={!step1Ok} style={{ ...btnPrimary, marginTop: 22, opacity: step1Ok ? 1 : 0.45 }}>
                  Continue →
                </button>
              </div>
            )}
              ...
              <div className="step-in">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <Field label="Pharmacy Name *">
                    <input type="text" placeholder="e.g. Negombo Central Pharmacy" value={form.pharmacy_name} onChange={set("pharmacy_name")} onFocus={() => setFocused("pn")} onBlur={() => setFocused(null)} style={{ ...inputSt, ...fb("pn") }} />
                  </Field>

                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <Field label="Contact Number">
                      <input type="tel" placeholder="+94 31 222 1234" value={form.contact_number} onChange={set("contact_number")} onFocus={() => setFocused("cn")} onBlur={() => setFocused(null)} style={{ ...inputSt, ...fb("cn") }} />
                    </Field>
                    <Field label="WhatsApp Number">
                      <input type="tel" placeholder="+94 77 123 4567" value={form.whatsapp_number} onChange={set("wn")} onFocus={() => setFocused("wn")} onBlur={() => setFocused(null)} style={{ ...inputSt, ...fb("wn") }} />
                    </Field>
                  </div>

                  <Field label="Address">
                    <textarea placeholder="e.g. 45 Main Street, Negombo" value={form.address} onChange={set("address")} onFocus={() => setFocused("addr")} onBlur={() => setFocused(null)}
                      style={{ ...inputSt, resize: "vertical", minHeight: 72, ...fb("addr") }} />
                  </Field>
                </div>

                <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
                  <button onClick={() => setStep(1)} style={btnGhost}>← Back</button>
                  <button onClick={() => step2Ok && setStep(3)} disabled={!step2Ok} style={{ ...btnPrimary, flex: 1, opacity: step2Ok ? 1 : 0.45 }}>Continue →</button>
                </div>
              </div>
            )}

            {/* ══ STEP 3 — Policies & hours ══ */}
            {step === 3 && (
              <div className="step-in">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <Field label="Opening Hours">
                      <input type="text" placeholder="e.g. Mon–Sat 8am–8pm" value={form.opening_hours} onChange={set("opening_hours")} style={{ ...inputSt }} />
                    </Field>
                    <Field label="Est. Delivery Time">
                      <input type="text" placeholder="e.g. 1–2 hours" value={form.estimated_delivery_time} onChange={set("estimated_delivery_time")} style={{ ...inputSt }} />
                    </Field>
                  </div>

                  <Field label="Service Areas">
                    <input type="text" placeholder="e.g. Negombo, Wattala, Ja-Ela" value={form.service_areas} onChange={set("service_areas")} style={{ ...inputSt }} />
                  </Field>

                  <Field label="Prescription Policy">
                    <textarea placeholder="e.g. Valid prescription required for all Schedule H drugs…" value={form.prescription_policy} onChange={set("prescription_policy")}
                      style={{ ...inputSt, resize: "vertical", minHeight: 72 }} />
                  </Field>

                  <Field label="Refund Policy">
                    <textarea placeholder="e.g. No returns accepted on dispensed medications…" value={form.refund_policy} onChange={set("refund_policy")}
                      style={{ ...inputSt, resize: "vertical", minHeight: 72 }} />
                  </Field>
                </div>

                {error && (
                  <div style={{ background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 10, padding: "11px 14px", marginTop: 16 }}>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label style={{ display: "block", color: "#64748b", fontSize: 11.5, fontWeight: 700, letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 7 }}>{label}</label>
      {children}
    </div>
  );
}
...
const card: React.CSSProperties = { background: "rgba(10,20,42,0.85)", backdropFilter: "blur(20px)", border: "1px solid rgba(148,163,184,0.09)", borderRadius: 24, boxShadow: "0 32px 80px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.04)", padding: "40px 38px" };
const logoBox: React.CSSProperties = { width: 38, height: 38, borderRadius: 11, background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.2)", display: "flex", alignItems: "center", justifyContent: "center" };
...
const inputSt: React.CSSProperties = { width: "100%", background: "rgba(6,13,26,0.9)", border: "1px solid rgba(148,163,184,0.1)", borderRadius: 11, color: "#f1f5f9", padding: "11px 14px", fontSize: 13.5, fontFamily: "'DM Sans',sans-serif", outline: "none", boxSizing: "border-box", transition: "border-color .2s,box-shadow .2s" };
...
const btnGhost: React.CSSProperties = { background: "transparent", border: "1px solid rgba(148,163,184,.12)", color: "#94a3b8", borderRadius: 12, padding: "13px 18px", fontSize: 13.5, fontFamily: "'DM Sans',sans-serif", cursor: "pointer", whiteSpace: "nowrap" as const };
...
const CSS = `
  ...
  input:focus,textarea:focus{border-color:rgba(56,189,248,.5)!important;box-shadow:0 0 0 3px rgba(56,189,248,.07)!important}
