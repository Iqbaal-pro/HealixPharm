...
export default function BatchesPage() {
  ...
  const fetchBatches = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`${API}/batches/?include_expired=${showExpired}`);
  ...
  const handleDeactivate = async (batchId: number) => {
    ...
    try {
      const res = await fetch(`${API}/batches/${batchId}/deactivate`, { method: "PATCH" });
  ...
  return (
    <div style={{ padding:"28px", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:20 }}>
        ...
        <span style={{ color:"#1e3a5f" }}>›</span>
        <span style={{ color:"#38bdf8", fontSize:13, fontWeight:600 }}>Batches</span>
      </div>
      ...
      <div className="fade-1" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28, flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          <div style={{
            ...
            color:"#38bdf8", boxShadow:"0 0 18px rgba(56,189,248,0.1)", flexShrink:0,
          }}>
          ...
          <div>
            ...
            <p style={{ color:"#475569", fontSize:13.5, margin:"3px 0 0" }}>
              Manage stock batches, expiry dates & FEFO ordering
            </p>
          </div>
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <button onClick={() => setShowExpired(p => !p)} style={{
            padding:"9px 16px", borderRadius:10, fontSize:13, border:"none", cursor:"pointer",
            fontFamily:"'DM Sans',sans-serif", fontWeight:500,
            background: showExpired ? "rgba(100,116,139,0.15)" : "rgba(148,163,184,0.06)",
            color: showExpired ? "#94a3b8" : "#475569",
          }}>
      ...
      {!loading && counts.critical > 0 && (
        <div className="fade-3" style={{
          ...
          <span style={{ fontSize:13, color:"#f87171", fontWeight:500 }}>
            {counts.critical} batch{counts.critical > 1 ? "es" : ""} expiring within 30 days — prioritise dispensing
          </span>
        </div>
      )}
      ...
      <div className="fade-3 glass-panel">
        ...
        <div style={{ padding:"18px 24px 14px", display:"flex", justifyContent:"space-between", alignItems:"center", borderBottom:"1px solid rgba(148,163,184,0.06)", flexWrap:"wrap", gap:12 }}>
          <div>
            <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:15, color:"#f1f5f9", margin:0 }}>All Batches</h2>
            <p style={{ fontSize:12, color:"#475569", marginTop:3 }}>
              {loading ? "Loading..." : `${filtered.length} batch${filtered.length !== 1 ? "es" : ""}`}
            </p>
          </div>
        ...
        {error && (
          <div style={{ margin:"16px 24px", padding:"12px 16px", background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:10, color:"#f87171", fontSize:13 }}>
            ⚠ {error} — <span style={{ cursor:"pointer", textDecoration:"underline" }} onClick={fetchBatches}>retry</span>
          </div>
        )}
                  ...
                  return (
                    <tr key={b.id} className="tr-hover" style={{ borderBottom:"1px solid rgba(148,163,184,0.04)" }}>
                      ...
                      <td style={{ padding:"13px 20px" }}>
                        <span className="badge" style={{ background:"rgba(14,165,233,0.08)", color:"#7dd3fc", fontFamily:"monospace" }}>MED-{b.medicine_id}</span>
                      </td>
                      <td style={{ padding:"13px 20px", fontSize:13, color:"#64748b" }}>{b.supplier_id ?? "—"}</td>
                      <td style={{ padding:"13px 20px", fontSize:13, color:"#64748b", whiteSpace:"nowrap" }}>
                        {new Date(b.received_date).toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" })}
                      </td>
                      <td style={{ padding:"13px 20px", fontSize:13, color:"#64748b", whiteSpace:"nowrap" }}>
                        {new Date(b.manufacture_date).toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" })}
                      </td>
                      <td style={{ padding:"13px 20px", fontSize:13, color: days <= 90 ? "#f97316" : "#94a3b8", whiteSpace:"nowrap" }}>
                        {new Date(b.expiry_date).toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" })}
                      </td>
                      <td style={{ padding:"13px 20px" }}>
                        <span className="badge" style={{
                          background: b.is_expired ? "rgba(100,116,139,0.08)" : days <= 30 ? "rgba(239,68,68,0.08)" : days <= 90 ? "rgba(249,115,22,0.08)" : "rgba(74,222,128,0.08)",
                          color:      b.is_expired ? "#64748b"                : days <= 30 ? "#ef4444"              : days <= 90 ? "#f97316"              : "#4ade80",
                        }}>
                          {b.is_expired ? "Expired" : `${days}d`}
                        </span>
                      </td>
                      <td style={{ padding:"13px 20px", fontSize:13, color:"#64748b" }}>
                        LKR {(b.cost_price * 300).toFixed(2)}
                      </td>
                      <td style={{ padding:"13px 20px" }}>
                        ...
                        {b.is_active && !b.is_expired && (
                          <button
                            ...
                            style={{
                              ...
                              opacity: approvingId === b.id ? 0.6 : 1,
                            }}>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={10} style={{ padding:"48px 24px", textAlign:"center", color:"#334155", fontSize:13 }}>
                      {batches.length === 0 ? "No batches found — create your first batch" : "No batches match your filter"}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div style={{ padding:"14px 24px", borderTop:"1px solid rgba(148,163,184,0.06)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <span style={{ color:"#334155", fontSize:12 }}>
              FEFO enforced — earliest expiring batches are always dispatched first.
            </span>
          </div>
          {!loading && (
            <span style={{ fontSize:12, color:"#334155" }}>{batches.length} total batches</span>
          )}
        </div>
      </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                {([
                  ...
                  { label:"Quantity Received", key:"quantity_received",  placeholder:"e.g. 200"     },
                ] as const).map(f => (
                  <div key={f.key} style={{ display:"flex", flexDirection:"column", gap:6 }}>
                    <label style={{ fontSize:13, color:"#94a3b8", fontWeight:500 }}>{f.label}</label>
                    <input className="input-field" placeholder={f.placeholder}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                <label style={{ fontSize:13, color:"#94a3b8", fontWeight:500 }}>Cost Price (per unit)</label>
                <input className="input-field" placeholder="e.g. 0.05"
                  value={form.cost_price} onChange={e => setForm(p => ({ ...p, cost_price: e.target.value }))} />
              </div>
              ...
              <button className="btn-primary" style={{ marginTop:4 }} onClick={handleCreateBatch} disabled={submitting}>
                {submitting
                  ? <><div style={{ width:14, height:14, border:"2px solid rgba(255,255,255,0.3)", borderTop:"2px solid #fff", borderRadius:"50%", animation:"spin 0.8s linear infinite" }}/> Creating...</>
                  : "Create Batch"
                }
              </button>
            </div>
