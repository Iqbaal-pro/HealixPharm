...
function EmptyState({ filters, onReset }: { filters: { spec: string; hospital: string; name: string }; onReset: () => void }) {
  const applied = [
  ...
  return (
    <div className="glass animate-fade-up" style={{ padding: "48px 32px", textAlign: "center" }}>
      ...
      <p style={{ color: "#475569", fontSize: 14, maxWidth: 320, margin: "0 auto 24px", lineHeight: 1.7 }}>We couldn't find any doctors matching your search. Try adjusting your filters.</p>
      {applied.length > 0 && (
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 36, flexWrap: "wrap", gap: 12 }}>
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", borderRadius: 8, background: "rgba(148,163,184,0.06)", border: "1px solid rgba(148,163,184,0.1)", color: "#94a3b8", fontSize: 13, fontWeight: 500, cursor: "pointer" }}
        >
          ...
          {stepLabels.map((label, i) => {
            ...
            return (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "5px 12px", borderRadius: 99, background: active ? "rgba(99,102,241,0.1)" : "transparent", border: active ? "1px solid rgba(99,102,241,0.22)" : "1px solid transparent" }}>
                  <div style={{ width: 18, height: 18, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, background: active ? "linear-gradient(135deg, #38bdf8, #818cf8)" : done ? "rgba(56,189,248,0.15)" : "rgba(148,163,184,0.08)", color: active ? "#fff" : done ? "#38bdf8" : "#334155", flexShrink: 0 }}>{done ? "✓" : i + 1}</div>
                  <span style={{ fontSize: 12, fontWeight: active ? 600 : 400, color: active ? "#a78bfa" : done ? "#475569" : "#2d3f55" }}>{label}</span>
                </div>
                {i < stepLabels.length - 1 && <div style={{ width: 24, height: 1, background: done ? "rgba(56,189,248,0.2)" : "rgba(148,163,184,0.08)" }} />}
              </div>
            );
          })}
        </div>
          ...
          <div className="animate-fade-up" style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 20 }}>
            {loading ? (
              ...
              <>
                ...
                {(spec || hospital || name) && <span style={{ color: "#334155", fontSize: 13 }}>{[spec, hospital, name].filter(Boolean).join(" · ")}</span>}
              </>
            )}
          </div>
          {error && <ErrorBanner message={error} onRetry={loadDoctors} />}
          {loading && !error && <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{[1,2,3].map(i => <SkeletonCard key={i} />)}</div>}
          {!loading && !error && results.length === 0 && <EmptyState filters={{ spec, hospital, name }} onReset={() => router.push("/channel")} />}
      ...
      {step === "book" && selectedDoctor && selectedSlot && selectedSlotId && (
        <BookingForm
          doctor={selectedDoctor}
          slot={selectedSlot}
...
export default function ResultsPage() {
  return (
    <Suspense fallback={<div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}><div style={{ color: "#475569", fontSize: 13 }}>Loading...</div></div>}>
      <ResultsContent />
    </Suspense>
  );
}